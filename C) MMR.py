...
# Load data
vac_coverage_df = pd.read_csv(r"C:\MMR-py\vac coverage 2025-04-03 15-07 UTC.csv", encoding='ISO-8859-1', on_bad_lines='skip', delimiter=';')
reported_cases_df = pd.read_csv(r"C:\MMR-py\reported cases and incidence 2025-04-03 15-17 UTC INC.csv", encoding='ISO-8859-1', on_bad_lines='skip', delimiter=';')

filename_plot_text = "Dowhy causal estimate on mean vac coverage and cases"
...
log_file_path = fr"C:\MMR-py\C) {filename_plot_text} valid years for dowhy calc {year_range_text}.txt"
...
if len(causal_effects) > 0:
    ...
    fig.update_layout(
        title=f"Dowhy Causal Effect estimation Vaccination Coverage on Reported Cases ({year_range_text})",
        xaxis_title="Country",
        showlegend=True,
    ...
    # Save the plot
    fig.write_html(fr"C:\MMR-py\C) {filename_plot_text} {year_range_text}.html")

    # Show the plot
    fig.show()

    print(fr"C:\MMR-py\C) {filename_plot_text} {year_range_text}.html has been saved.")
else:
